...
TEST(InterfaceTest, TestGlobalTensorAssignment)
{
  ...
  tensor.setZero();
  tensor(0,0,0) = 1;
  std::array<int, 3> dims = {(int)tensor.dimension(0), (int)tensor.dimension(1), (int)tensor.dimension(2)};
  int n_dims = 3;
...
extern "C" {
  void f_return_string(const char* c_string, int string_length);
}
TEST(InterfaceTest, TestInterfaceString) {
  std::string teststring = "test string";
  int string_length = std::size(teststring);
  f_return_string(teststring.c_str(), string_length);
}
